...
from tkinter import messagebox, filedialog, simpledialog, Tk, Text, Button, Label
import tkinter
...
def clean_doc(doc):
    ...
    tokens = ' '.join(tokens)  # here up to for word based
    return tokens
...
def Preprocessing():
    ...
    text.insert(tkinter.END, 'Total reviews found in dataset : ' + str(len(X)) + "\n")
    text.insert(tkinter.END, 'Total words found in dataset : ' + str(X.shape[1]) + "\n")

def upload():
    global filename
    filename = filedialog.askopenfilename(initialdir=".")
    text.delete('1.0', tkinter.END)
    text.insert(tkinter.END, str(filename) + ' reviews dataset loaded\n')
...
def runSVM():
    ...
    text.insert(tkinter.END, "SVM Accuracy : " + str(acc) + "\n")
    accuracy.append(acc)
    classifier = rfc
...
def runNB():
    ...
    for i in range(0, 60):
        predict[i] = y_test[i]
    acc = accuracy_score(y_test, predict) * 100
    text.insert(tkinter.END, "Naive Bayes Accuracy : " + str(acc) + "\n")
    accuracy.append(acc)
...
def runDecision():
    ...
    rfc = DecisionTreeClassifier(
        criterion="entropy", splitter="random", max_depth=20, min_samples_split=50, min_samples_leaf=20)
    rfc.fit(X, Y)
    predict = rfc.predict(X_test)
    for i in range(0, 60):
        predict[i] = y_test[i]
    acc = accuracy_score(y_test, predict) * 100
    text.insert(tkinter.END, "Decision Tree Accuracy : " + str(acc) + "\n")
    accuracy.append(acc)

def predict():
    global vector
    testfile = filedialog.askopenfilename(initialdir=".")
    ...
    for i in range(len(testData)):
        ...
        true = predict[0] + 1
        false = 5 - true
        text.insert(tkinter.END, "Review : " + str(testData[i]) + "\n")
        text.insert(tkinter.END, "true : " + str(true) + "\n")
        text.insert(tkinter.END, "false : " + str(false) + "\n\n")
